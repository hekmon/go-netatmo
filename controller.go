...
func NewClientWithAuthorizationCode(ctx context.Context, conf OAuth2Config, authCode string, customClient *http.Client) (client Client, err error) {
...
// customClient can be nil
func NewClientWithClientCredentials(ctx context.Context, conf OAuth2Config, username, password string, customClient *http.Client) (client Client, err error) {
	// Prepare the oauth2 enabled client
	c := prepareController(ctx, conf, customClient)
	// Exchange auth code for access & refresh token
...
func NewClientWithToken(ctx context.Context, conf OAuth2Config, previousTokens *oauth2.Token, customClient *http.Client) (client Client, err error) {
	// Prepare the oauth2 enabled client
	c := prepareController(ctx, conf, customClient)
...
func prepareController(ctx context.Context, conf OAuth2Config, customClient *http.Client) (c *Controller) {
